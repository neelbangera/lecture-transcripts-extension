...
4. For each queued job, derive both write-once paths: `<courseSlug>/lectures/<NNN>.md` and `<courseSlug>/timestamped/<NNN>.md` with NNN zero-padded 3 digits (e.g. `eecs491/lectures/006.md`). The metadata block `lecture:` stays integer (e.g. `6`), `term:` stays the normalized term, filename uses the padded number only.
...
The generated lecture files are (plain at `<courseSlug>/lectures/<NNN>.md`, timestamped at `<courseSlug>/timestamped/<NNN>.md`):
