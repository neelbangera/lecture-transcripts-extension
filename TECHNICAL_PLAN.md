...
plain path: "<courseSlug>/lectures/<lectureNumber:03d>.md", e.g. "eecs491/lectures/006.md"
timestamped path: "<courseSlug>/timestamped/<lectureNumber:03d>.md", e.g. "eecs491/timestamped/006.md"
plain-only path: "<courseSlug>/<lectureNumber:03d>.md", used only when the job has no timestamped form; no timestamped file is written
...
- Publish sanitization (omit): if transport-valid but lowercased path segments (split on `/._-`) contain whole segment `token|session|auth|sid`, omit `source_url` from the metadata block (still store canonical host+path in queue for dedup). Logs store host+path only, never query/fragment.
...
4. Implement the transcript completion observer in `extension/src/content.ts`. It should begin on load of a recognized lecture page or on an in-page URL change: open the transcript control when the transcript is closed, or use the explicitly defined already-expanded-and-populated initial state, wait for the fixture-defined completion indicator, attach `MutationObserver` only to the transcript container subtree, take two normalized snapshots separated by a 1500ms no-mutation debounce, enforce a 30s overall timeout -> `not_ready`, require >=50 non-whitespace chars, and inspect loading markers only inside the fixture-defined loading/error region. Stop observing after two matching snapshots or a terminal rejection. A `not_ready` result must be recoverable by a later reload or in-page URL change, and must not be retried forever without a new activation.
...
2. Implement `extension/src/content.ts` as the page-facing coordinator. It must activate on recognized lecture-page load and in-page URL change, open the transcript control when needed, call the parser, and send only schema-shaped messages to the service worker.
...
The generated lecture files are (plain at `<courseSlug>/lectures/<NNN>.md`, timestamped at `<courseSlug>/timestamped/<NNN>.md`; a job with no timestamped form writes only `<courseSlug>/<NNN>.md`):
...
- A new lecture creates exactly two remote Markdown files (one commit per file) and no other change.
...
| `extension/src/content.ts` | Create | Runs on Leccap pages, activates on recognized lecture-page load or URL change, observes loading, and submits extracted jobs. |
...
| `uploader/internal/markdown/render.go` | Create | Renders the plain and timestamped documents with the shared bottom metadata block. |
...
The uploader may create one plain and one timestamped remote file per successfully captured lecture at:

```text
neelbangera/lecture-transcripts/
  <courseSlug>/lectures/<NNN>.md      (plain; metadata lecture: 6 at the bottom)
  <courseSlug>/timestamped/<NNN>.md   (timestamped; same metadata block)
```
