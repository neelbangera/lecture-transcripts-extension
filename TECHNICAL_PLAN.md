...
4. Implement the transcript completion observer in `extension/src/content.ts`. It should begin on load of a recognized lecture page or on an in-page URL change: open the transcript control when the transcript is closed (retrying until the client-rendered control exists), close it again after a terminal outcome when the run opened it, or use the explicitly defined already-expanded-and-populated initial state, wait for the fixture-defined completion indicator, attach `MutationObserver` only to the transcript container subtree, take two normalized snapshots separated by a 1500ms no-mutation debounce, enforce a 30s overall timeout -> `not_ready`, require >=50 non-whitespace chars, and inspect loading markers only inside the fixture-defined loading/error region. Stop observing after two matching snapshots or a terminal rejection. A `not_ready` result must be recoverable by a later reload or in-page URL change, and must not be retried forever without a new activation.
...
2. Implement `extension/src/content.ts` as the page-facing coordinator. It must activate on recognized lecture-page load and in-page URL change, open the transcript control when needed (and close it after a terminal outcome when it opened it), call the parser, and send only schema-shaped messages to the service worker.
...
The generated lecture files are (plain at `<courseSlug>/<NNN>.md`, timestamped at `<courseSlug>/timestamped/<NNN>.md`; a job with no timestamped form writes only the plain file):
...
neelbangera/lecture-transcripts/
  <courseSlug>/<NNN>.md               (plain; metadata lecture: 6 at the bottom)
