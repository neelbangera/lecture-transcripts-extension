...
- The repository is no longer design-only: the package manifests, Go module, queue DDL, canonical schema files, normalization-vector file, packaging scripts, and tests are in the tree. The Go executable entrypoint (`uploader/cmd/lecture-uploader`) and the serial processor (`uploader/internal/processor`) have not landed yet, so `scripts/build-uploader.sh` fails closed until they do.
