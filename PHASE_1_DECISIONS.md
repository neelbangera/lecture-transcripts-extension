...
- On 2026-09-22 the owner selected a two-file, term-free layout for the personal repository: the plain transcript at `<courseSlug>/<NNN>.md` and the timestamped transcript at `<courseSlug>/timestamped/<NNN>.md`. Each document carries the same metadata block (course, term, lecture, date, optional source_url, captured_at, transcript_sha256) at the bottom, and the timestamped document renders exactly one line per timestamp entry. The term remains part of the lecture identity and lectureKey but not of the repository path because the repository holds one term. A lecture with no timestamped form writes only the plain file. The create-commit message is `Add <courseName> lecture <N> (<path>)`. The hash framing and normalization are unchanged, so previously captured content stays hash-identical; the uploader publishes the plain file first and the timestamped file second, and a retry completes a partially created pair. Legacy combined files were deleted by the owner and recaptured.
- On 2026-09-22 the owner added discussion capture. The observed overview lists discussion recordings with titles like `Discussion 1` (badge `Discussion - 012`), while decoys such as `DISREGARD -- Empty discussion` and `Lecture recorded on 9/18/2026` fail closed. Transcript jobs therefore carry `kind: "lecture" | "discussion"`; the numeric identity and lectureKey are unchanged, and kind disambiguates queue rows and paths. Discussion paths are `<courseSlug>/discussions/<NNN>.md` and `<courseSlug>/discussions/timestamped/<NNN>.md`. A discussion is first-capture-wins: once any queue row exists for that kind+lectureKey, later sections of the same discussion are acknowledged as duplicates even if their transcript differs, so write-once conflicts do not fire for the same discussion number. EECS 491 Fall 2026 was added to the allowlist for this purpose.
- On 2026-09-22 the owner changed activation so a recognized lecture page captures on load and on in-page URL change instead of requiring a manual transcript click: the content script opens the transcript control itself when the transcript is closed (retrying until the client-rendered control appears), closes it again after the capture reaches a terminal outcome when it opened it, and captures an already-open populated transcript directly. Non-lecture pages still do nothing, and the completion contract, stable-snapshot rule, timeout, and dedup rules are unchanged.
