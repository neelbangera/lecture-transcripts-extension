...
- The repository is currently design-only and has no package manifests, Go module, queue DDL, canonical schema files, normalization-vector file, or rendered host manifest. Their exact contracts are now written in `TECHNICAL_PLAN.md`; the implementation files are outputs after the readiness gate, not evidence that the gate has passed.
...
No implementation file may be created until the Stage 0 packet and machine-local provisioning packet described in `TECHNICAL_PLAN.md` exist and pass validation. In particular, a later agent must not fill missing selectors with guessed CSS, treat the example course as the complete course set, hardcode fake account identifiers, or proceed with unmeasured limits.
